...
The material comes from the Carolingian Canon Law project \citep[CCL;][]{firey2009ccl}, an open-access digital edition of early medieval canon-law manuscripts, most of them ninth-century and never edited to modern standards. The project transcribes the manuscripts, almost all by hand, and keys each transcribed canon or excerpt to its source where it is identified: a conciliar canon, a papal decretal, a royal capitulary or a patristic excerpt. We adapt that material into a benchmark. A directory is one labelled fragment, named by its CCL source key, and a file is one manuscript witness of that fragment (one manuscript's copy of it). The transcriptions and the key labels are the project's work; we derive the plain-text files from its TEI-P5 exports (the Text Encoding Initiative's XML standard for scholarly editions) with the labels as supplied, and the split and the two tasks are ours. Appendix~\ref{app:corpus_provenance} gives the transcription workflow, the label taxonomy and the derivation rule.
...
\paragraph{The project.} The Carolingian Canon Law project \citep[CCL;][]{firey2009ccl} is an open-access digital edition founded in 2009; \citet{eichbauer2014ccl} reviews it.\footnote{Founding year, pre-2019 workflow, proofreading standard and the one manuscript transcribed by handwritten text recognition (HTR): project director, personal communication, 2026.} Its texts are early medieval canon-law manuscripts, most of them ninth-century and never edited to modern standards. Each manuscript is identified by its siglum (Place-Library-Shelfmark), and a witness is one manuscript's copy of one unit (one transcribed canon or excerpt).
...
\paragraph{Labels.} CCL keys every unit to its original source, such as a fourth-century council or a papal decretal, and our directories reuse that key as supplied. A key is a four-letter source code, a year and a canon or chapter number, as in \texttt{CTOU.567.16}; some labels carry a trailing annotation, as in \texttt{CTOU.567.16 (15)} or \texttt{CARL.501?.18}. Counting a directory as key-named when its label begins with a four-letter code, a period, a year and a period, whatever follows, 689 of the 840 directory labels are source keys. The other 151 are modern edition citations, the Apostolic Canons (\texttt{Can.apost.}$N$), references to other collections such as the Capitula Martini, biblical references, and a handful of unidentified sources; each unidentified source is a singleton directory and contributes no positive pairs. The assignment of witnesses to keys was carried out by the project; we report the keys as supplied and do not have a description of the annotators, their instructions or the verification applied.
