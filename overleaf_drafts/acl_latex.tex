...
Tables~\ref{tab:taskb} and~\ref{tab:taskB_ranking_appendix_mseed} in Appendix~\ref{app:sif_variants} repeat the top-1 comparison under five query and reference reseedings at the train-selected layers of Table~\ref{tab:taskB_headline}, so the compression is not an artifact of one draw. The baseline spread of 36.6 points, from 50.7 for mT5-base to 87.3 for KaLM-mini, falls to 1.7 points under SIF+ABTT, from 88.9 for LaTa to 90.6 for LaBSE, with standard deviations of at most 1.0. Table~\ref{tab:taskb} gives the shortlist view: every model exceeds 95\% by Top-2 and gains little after Top-3, so two or three options cover nearly all successful cases.
