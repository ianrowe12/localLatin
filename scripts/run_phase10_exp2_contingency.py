"""Phase 10 Experiment 2 Contingency: Last-token pooling + Optimal Transport.

Copy of Phase 9 contingency with three targeted changes:
  A. --half_precision arg (apply torch.float16 when loading model)
  B. --phase9_results_csv LaBSE fallback (load LaBSE reference from Phase 9 if not in current results)
  C. Save pair_sims to cache dir for bias analysis
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 10 Exp 2 contingency experiments.")
    ...
    parser.add_argument("--cache_dir", default="", help="Cache dir for pair_sims (defaults to out_dir/cache).")
    ...
    # Change A: half precision
    parser.add_argument("--half_precision", action="store_true",
                        help="Load model in float16 (required for 8B+ models).")
    # Change B: Phase 9 LaBSE fallback
    parser.add_argument("--phase9_results_csv", default="",
                        help="Path to Phase 9 results.csv for LaBSE reference if not in current results.")
...
def save_pair_sims(cache_dir: Path, slug: str, language: str, layer: int,
                   name: str, sims: np.ndarray) -> None:
    """Save pair similarities for bias analysis."""
    out_path = cache_dir / slug / language / f"layer{layer}_{name}_pair_sims.npy"
...
def main() -> None:
    ...
    # Change B: LaBSE baseline with Phase 9 fallback
    labse_rows = results_df[
        (results_df["model"] == "sentence-transformers/LaBSE") &
        (results_df["method"] == "baseline_mean")
    ]
    if labse_rows.empty and args.phase9_results_csv:
        p9 = pd.read_csv(args.phase9_results_csv)
        labse_rows = p9[
            (p9["model"] == "sentence-transformers/LaBSE") &
            (p9["method"] == "baseline_mean")
        ]
        print("[INFO] LaBSE reference loaded from Phase 9 results.")
    ...
    for model_name in decoder_models:
        ...
        for language in languages:
            ...
            # Change C: Save pair_sims for bias analysis
            save_pair_sims(cache_dir, slug, language, best_layer, "lasttok", sims_lt)
            ...
            # Change C: Save pair_sims for bias analysis
            save_pair_sims(cache_dir, slug, language, best_layer, "lasttok_abtt", sims_lt_abtt)
            ...
            if not args.skip_ot:
                try:
                    ...
                    # Change C: Save pair_sims for bias analysis
                    save_pair_sims(cache_dir, slug, language, best_layer, "ot", ot_sims)
