"""Phase 10 Experiment 2: MUSTS 50/50 train/test split for Spanish only.

The 5 existing language CSVs from Phase 9 are copied via cp -n in the sbatch.
This script handles Spanish (the new 6th language).

Output:
- runs/phase10/experiment2/spanish_split.csv
  columns: sentence_1, sentence_2, similarity, split
- runs/phase10/experiment2/split_summary.csv  (appended or created)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 10 Exp 2 data prep (Spanish only).")
    parser.add_argument("--out_dir", default="runs/phase10/experiment2")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument(
        "--languages", default="spanish",
        help="Comma-separated language names (default: spanish).",
    )
...
def main() -> None:
    ...
    # Append to existing summary if present, else create
    if summary_path.exists():
        existing = pd.read_csv(summary_path)
        existing = existing[~existing["language"].isin(summary["language"])]
