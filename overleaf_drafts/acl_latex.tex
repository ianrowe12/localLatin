...
The corpus contains 1,705 Medieval Latin canon-law witnesses (manuscript copies of a text) from the Carolingian Canon Law project (CCL), an ongoing digital edition founded in 2009 \citep{firey2009ccl, eichbauer2014ccl}. Its manuscripts are mostly ninth-century, and most have never been edited to modern standards. Transcription is manual, mostly from manuscript images and for some texts from eighteenth-century printings. Before 2019 transcribers worked from microfilm and CCL contributors converted the files to TEI-P5, the Text Encoding Initiative's XML standard for scholarly editions; since 2019 they work in an editor beside a IIIF image of the folio (a zoomable web-served scan of the manuscript leaf), and an approved proofreader then verifies each transcription against the image to the letter and adds TEI markup. The interface credits the transcriber and proofreader of each unit (one transcribed canon or excerpt). We hold the witnesses as plain text with the labels as supplied, and we are confirming with CCL how they were extracted from the TEI.

CCL keys every unit to its original source, such as a fourth-century council or a papal decretal, and our directories reuse that key. Most of our 840 directories are named by such keys; a minority are named by a modern edition citation or a biblical reference. A handful carry passages whose source is unidentified; each of these is a singleton and contributes no positive pairs. The files in a directory are the surviving witnesses to one labelled fragment, each free to vary from the others, so the benchmark asks whether an unidentified witness belongs to a known fragment. Directory sizes range from one to ten files: variants inside a directory count as positive matches, and files from different directories count as negatives.
...
Tables~\ref{tab:taskb} and~\ref{tab:taskB_ranking_appendix_mseed} in Appendix~\ref{app:sif_variants} repeat the top-1 comparison under five query and reference reseedings, so the compression is not an artifact of one draw. The baseline spread of 36.6 points across best baseline layers, from 50.7 for mT5-base to 87.3 for KaLM-mini, falls to 1.1 points under the best per-model SIF+ABTT configuration, from 89.5 for KaLM-mini to 90.6 for LaBSE, with standard deviations of at most 1.1 points. Table~\ref{tab:taskb} gives the scholar-facing shortlist view: every model exceeds 95\% by Top-2 and gains little after Top-3, so two or three options cover nearly all successful cases.
...
Six models across three architectures narrow what we can claim. Anisotropy is universal on this corpus and so is the repair, but the mid-depth retrieval collapse is not. Layerwise profiles place the mid-depth anisotropy peak in decoder-only models and do not separate encoder-decoder encoders \citep{razzhigaev2024shape, godey2024anisotropy}, so we read this as a first report for T5 encoders, not a property of the architecture. A single-architecture study could report that collapse as the general finding.
