...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract FF1 embeddings for LaTa-style models.")
...
def pool_ff1(
    ...
    if pooling == "mean":
        ...
        return (ff_act * mask).sum(dim=1) / denom
    lengths = attention_mask.sum(dim=1) - 1
    lengths = lengths.clamp(min=0)
    batch_idx = torch.arange(ff_act.size(0), device=ff_act.device)
    if pooling == "lasttok":
        return ff_act[batch_idx, lengths]
    if pooling != "sif":
    ...
    weights = torch.from_numpy(weights_np).to(ff_act.device)
...
def main() -> None:
    ...
    layers = parse_layers(args.layers) or list(range(1, total_layers + 1))
    if any(layer < 1 or layer > total_layers for layer in layers):
        raise ValueError(f"Layers must be within 1..{total_layers}: {layers}")
    ...
    config = {
        ...
        "layer_indexing": {"ff1": "1..N encoder blocks"},
        "timestamp": run_id,
        "device": device,
    }
    ...
    for layer_num in layers:
        ...
        print(f"Extracting FF1 layer {layer_num} ({args.pooling})...")
        ...
        if args.pooling == "mean":
            ...
            suffix = "_sif"
        np.save(run_dir / f"ff1_layer{layer_num}_embeddings{suffix}.npy", emb)
        np.save(run_dir / f"ff1_layer{layer_num}_embeddings{suffix}_norm.npy", emb_norm)
