...
Resolved (issue #39): the provenance paragraph and citations are now in Section 2. The corpus comes from the Carolingian Canon Law project (CCL, <https://ccl.rch.uky.edu>), created by Prof. Abigail Firey at the University of Kentucky in 2009 and supervised by her since. There is no Latin department involvement. Transcriptions are made manually from manuscript images under published protocols, then pass a proofreading desk where an approved proofreader checks them to the letter and adds TEI-XML markup before publication; transcribers and proofreaders are credited per unit in the CCL interface. Directories in our corpus correspond to CCL source keys (the original text: a conciliar canon, papal decretal, royal capitulary or patristic excerpt), and files are witnesses, individual hand-copied manuscripts identified by siglum. Citations added: `firey2009ccl` (project) and `eichbauer2014ccl` (Digital Philology review of the project). Still open: licensing and usage terms, which Firey's email does not state and which must be asked directly. See question 1 below.
