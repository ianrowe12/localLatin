...
\paragraph{Task B.}
For autonomous routing, we report overall assignment accuracy, existing accuracy, and new accuracy. Assignment accuracy scores only the existing-versus-new decision: a test file with a partner in test is correct when its maximum cosine to any other test file is at least $\tau$, and a file without one is correct when that maximum falls below $\tau$. It does not check which directory the file would enter, so it has a floor at the 62.4 percent class prior and must be read next to directory accuracy at rank 1, which does require the correct directory. For top-$k$ ranking, cumulative top-$K$ accuracy is the fraction of test queries whose labelled directory appears among the top $K$ options.
...
The cosine gap column shows why. Under baseline pooling even the models with strong AUROC keep equivalent and non-equivalent passages within about 0.10 of each other, so the ranking is right but the score carries almost no margin for a threshold. ABTT restores that margin for all six.
