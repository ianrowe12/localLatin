...
extract_t5() {
  ...
  if [ ! -f "$RUNS_ROOT/phase9_bases/$slug/hidden_mean_tokempty/hidden_layer$(echo "$layers" | awk -F- '{print $2}')_embeddings.npy" ]; then
    $PYTHON_BIN src/extract_hidden_cli.py \
      --meta_csv "$META_CSV" \
      --runs_root "$RUNS_ROOT/phase9_bases" \
      --run_dir "$RUNS_ROOT/phase9_bases/$slug/hidden_mean_tokempty" \
  ...
  if [ ! -f "$RUNS_ROOT/phase9_bases/$slug/hidden_sif_tokempty/hidden_layer$(echo "$layers" | awk -F- '{print $2}')_embeddings_sif.npy" ]; then
    $PYTHON_BIN src/extract_hidden_cli.py \
      --meta_csv "$META_CSV" \
      --runs_root "$RUNS_ROOT/phase9_bases" \
      --run_dir "$RUNS_ROOT/phase9_bases/$slug/hidden_sif_tokempty" \
      --model_name "$model_name" \
      --layers "$layers" \
      --pooling sif \
      --token_filter tokenizer_empty \
...
mkdir -p "$PHASE11_DIR/distributions" "$PHASE11_DIR/figures"
...
$PYTHON_BIN scripts/run_phase11_evaluate.py \
  --split_csv "$SPLIT_CSV" \
  --runs_root "$RUNS_ROOT" \
  --out_csv "$PHASE11_DIR/phase11_results.csv" \
...
$PYTHON_BIN scripts/visualize_phase11_release.py \
  --results_csv "$PHASE11_DIR/phase11_results.csv" \
  --out_dir "$PHASE11_DIR/figures"
