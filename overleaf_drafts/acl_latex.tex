...
We study this problem as semantic matching on a benchmark of 840 labelled fragments of Medieval Latin canon law, each a canon or excerpt with one or more manuscript witnesses, 1,705 in all. Task A asks whether two passages express the same meaning. Task B turns the same embeddings into an open-set workflow: route a new passage to an existing directory, or mark it as new. Witnesses to one fragment are hand copies of one exemplar and differ mostly in spelling, so a character n-gram baseline matches the best embedding configuration on both tasks (Section~\ref{sec:results}); our question is not whether embeddings beat string matching but where inside a pre-trained encoder the retrieval signal lives, why it disappears, and what recovers it.

A final-layer vector cannot answer that, so we work layer by layer, and the layerwise view of six models separates two effects that are often reported together. Anisotropy is universal: every model has a layer where pooled passages crowd into a narrow cone, with mean cosine between randomly selected passages reaching 0.23 to 0.95, and no baseline layer of any model separates equivalent from non-equivalent passages by more than 0.29 in mean cosine (Figure~\ref{fig:gap}). A catastrophic mid-depth collapse of retrieval is not universal. LaTa, PhilTa, and mT5-base lose almost all pairwise separation in their middle layers, while LaBSE, Qwen3-0.6B, and KaLM-mini improve close to monotonically with depth.
...
In t-SNE \citep{vandermaaten2008tsne} and UMAP \citep{mcinnes2018umap} projections of all six models (Appendix~\ref{app:cluster_geometry}), ABTT-only with $D=10$ forms tighter same-directory groups than baseline mean pooling, with winnable-witness silhouette scores rising by 0.07 to 0.72 on the three T5 encoders.

\subsection{Task B: Directory Routing}

Per-layer routing and top-$k$ ranking tables for the two Task B variants of Section~\ref{sec:tasks} are in Appendices~\ref{app:per_layer_headline} and~\ref{app:per_layer_extra}.
...
Tables~\ref{tab:taskb} and~\ref{tab:taskB_ranking_appendix_mseed} in Appendix~\ref{app:sif_variants} repeat the top-1 comparison under five query and reference reseedings at the train-selected layers of Table~\ref{tab:taskB_headline}, so the compression is not an artifact of one draw. The baseline spread of 36.6 points, from 50.7 for mT5-base to 87.3 for KaLM-mini, falls to 1.7 points under SIF+ABTT, from 88.9 for LaTa to 90.6 for LaBSE, with standard deviations of at most 1.0. Table~\ref{tab:taskb} gives the shortlist view: every model exceeds 95\% by Top-2 and gains little after Top-3.

\paragraph{Lexical baselines set the operating point.}
Witnesses to one fragment are hand copies of one exemplar and differ mostly in spelling, so surface overlap is strong here. Character 3--5-gram TF-IDF matches the best ABTT cell on Task A (AUROC 0.987 for both) and is level with it on Task B (91.8 against 91.7 assignment accuracy, 89.9 against 89.4 directory accuracy at rank 1, one draw against seed standard deviations of up to 1.0). Stratified by TF-IDF overlap, the 596 positive test pairs show no regime where an embedding ranks better: TF-IDF is never worse in any tercile, leads LaBSE+ABTT corpus-wide (recall at rank 1 of 0.878 against 0.854, McNemar $p = 0.001$), leads only marginally on low-overlap pairs ($p = 0.023$ and $0.033$, uncorrected), and is at parity with LaBSE+ABTT's one routing edge there ($p = 0.33$). Word-level BM25 ranks well (AUROC 0.974) but routes no better than the 62.4 percent class prior, so the character baseline is the comparison that matters.

\paragraph{A parameter-free repair matches contrastive fine-tuning.}
Fine-tuning LaTa on the 565 positive train pairs gives the best uncorrected Task A row (AUROC 0.984) and 83.4 assignment accuracy; with ABTT on top it reaches 87.8 assignment accuracy and 85.2 directory accuracy at rank 1, below every zero-shot ABTT cell (88.5 to 91.7 and 86.1 to 89.4). ABTT also lowers that row's AUROC from 0.984 to 0.970 while widening its cosine gap from 0.387 to 0.548, consistent with fine-tuning having already removed part of the common component: the repair is not free on every row, and it matches what the available supervision buys.
...
ABTT improves rank faithfulness in all six model-view cells ($\rho_{\mathrm{LOO}}$ 6/6, with tie-corrected Kendall $\tau_b$ agreeing in all six), but chance-corrected deletion faithfulness improves in only three of six, and one of those three wins is within 1.2 standard errors of zero. The sufficiency-side metrics fail our shuffled-attribution control in two baseline cells and stay in the appendix, so we claim better-ranked rationales, not a gain on every axis.
