...
**Dataset provenance**: the corpus comes from the Carolingian Canon Law project (CCL, <https://ccl.rch.uky.edu>), created by Prof. Abigail Firey at the University of Kentucky in 2009 and supervised by her since. It is NOT from a Latin department; do not write that anywhere. Texts are early medieval (mostly 9th-century) canon-law manuscripts, most never edited to modern standards. Transcription is manual, mostly from manuscript images and for some texts from 18th-century printings. Pre-2019 transcribers worked from purchased microfilm into word-processed files that CCL contributors converted to TEI-P5; post-2019 they use IIIF images in the CCL Transcription Desk, after which an approved proofreader on the Proofreading Desk checks the text to the letter and adds TEI markup. Do not claim the proofreading stage covers the pre-2019 material. HTR has been attempted on exactly one manuscript (Paris, BnF, lat. 1454) and is still in proofreading, so the corpus is effectively but not entirely hand-transcribed. Most directories correspond to CCL source *keys* (the original text: a conciliar canon, papal decretal, royal capitulary, or patristic excerpt), but 151 of 840 labels are not key-named (rule and counts: `scripts/data/label_taxonomy.py`; a label is key-named when it begins with `CODE.year.`, a four-letter upper-case code, a period, a year and a period, whatever follows): modern edition citations, `Can.apost.N`, other collections such as `Capit.Martini.N`, biblical references, and a handful of unidentified sources (all singletons). Files are *witnesses*, individual hand copies identified by siglum (Place-Library-Shelfmark). Keys were assigned by the project director or by graduate students under her supervision who could check a manuscript version against the modern authoritative version, and they are verified by collation in CCL's parallel display of all units sharing a key, not by double annotation, so there is no inter-annotator agreement figure (`docs/research/benchmark_v1.md`). In the paper the labels are "expert-assigned by the project and checked by collation in its parallel display" and the benchmark is ours; never write "we annotate". We hold the texts as plain-text files; the derivation from CCL's TEI-P5 is known and scripted (`scripts/data/tei_to_canon.py`, see `docs/research/data_derivation.md`). The material is CC BY-NC-SA 4.0 with attribution to the project (director's email of 2026-09-15, issue #173, recorded in `data_derivation.md`); TEI exports are still not committed to this public repo, because release is coordinated with CCL's versioned exports.

The dataset is 1,278 `.txt` files in 538 directories under `data/canon/`, where each directory represents one original text. The labelled retrieval corpus is 1,705 files in 840 directories under `data/canon_labelled/`, and the unlabelled query set is 2,238 files under `data/canon_unlabelled/`. The labelled corpus is **frozen as benchmark v1** (issue #112): see `docs/research/benchmark_v1.md` for the manifest digest, the corrections log and the freeze rule (new CCL material goes to the webapp / v2, never into this paper's corpus). Active work is paper resubmission — see GAMEPLAN.md. Primary metric: Assignment Accuracy.
...
- **`scripts/data/`**: Corpus provenance tools (`tei_to_canon.py` converts a CCL TEI-P5 export to the plain-text layout and diffs it against `data/`; `benchmark_manifest.py` writes and checks the benchmark v1 sha256 manifest).
