...
Witnesses to one fragment are hand copies of one exemplar and differ mostly in spelling, so surface overlap is strong here. Character 3--5-gram TF-IDF (settings in Appendix~\ref{app:reference_systems}) matches the best ABTT cell on Task A (AUROC 0.987 for both) and is level with it on Task B (91.8 against 91.7 assignment accuracy, 89.9 against 89.4 directory accuracy at rank 1, one draw against seed standard deviations of up to 1.0). Stratified by TF-IDF overlap, the 596 positive test pairs show no regime where an embedding ranks better: TF-IDF is never worse in any tercile, leads LaBSE+ABTT corpus-wide (recall at rank 1 of 0.878 against 0.854, McNemar $p = 0.001$), leads only marginally on low-overlap pairs ($p = 0.023$ and $0.033$, uncorrected over twelve comparisons per stratum), and is at parity with LaBSE+ABTT's one routing edge there ($p = 0.33$). Word-level BM25 ranks well (AUROC 0.974) but routes no better than the 62.4 percent class prior, so the character baseline is the comparison that matters.
...
\section{Reference Systems: Lexical Baselines and the Fine-Tuning Ceiling}
\label{app:reference_systems}

Tables~\ref{tab:taskA_headline} and~\ref{tab:taskB_headline} carry four reference rows below the rule. Table~\ref{tab:lexical_baselines} gives the three lexical baselines with their settings: the BM25 parameters, the character $n$-gram range, the train-only fitting, and the min-max rescaling that precedes the threshold fit. Table~\ref{tab:finetune_ceiling} gives the supervised fine-tuning ceiling with the pre-trained rows it is compared against.
...
\onecolumn
\input{tables/lexical_baselines}
