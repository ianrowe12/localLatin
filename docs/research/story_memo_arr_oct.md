...
Gap: we still have no provenance paragraph and no citation. The draft currently describes the data without saying where it came from or who labelled it. This is the single most likely reviewer objection to a benchmark paper. See question 1 below.
...
Two facts carry this plank. First, anisotropy is present in all six. At each model's most collapsed layer the mean pairwise cosine is 0.226 (LaTa L4), 0.548 (PhilTa L6), 0.342 (mT5-base L5), 0.872 (LaBSE L8), 0.952 (Qwen3-0.6B L16) and 0.884 (KaLM-mini L5). ABTT with D=10 drives all six to roughly 0.00 and raises effective rank from as low as 1.00 to between 152 and 178. Source: `runs/active/resubmit/layer_diagnostics/geometry_per_layer.csv` (400 rows), summarised in Appendix Table `tab:layer_diagnostics_main`.

Second, the retrieval consequence is largest where the collapse is deepest, and the repair is uniform. Baseline mean-pooled AUROC bottoms out at 0.497 for LaTa (layer 6, chance), 0.539 for PhilTa (layer 10) and 0.654 for mT5-base (layer 5). ABTT at those same layers gives 0.963, 0.982 and 0.977. Source: `runs/active/resubmit/results/phase_resubmit_results.csv` (700 rows), rendered as Figures `fig:dip` and `fig:gap` and Table `tab:retrieval_summary_main`.

The cleanest headline number is Task B under five query and reference reseedings. Best-layer baseline top-1 accuracy spans 36.7 points across the six models, from 50.6 for mT5-base to 87.3 for KaLM-mini. Under SIF plus ABTT that spread collapses to 1.0 point, from 89.4 to 90.4, and every model clears 95 percent by top-2. Source: `runs/active/resubmit/taskb_mseed/aggregated_results.csv` (400 rows), rendered as Table `tab:taskb`. Per-layer tables live in `runs/active/resubmit/results/perlayer_tables/`.
...
**1. Dataset provenance and citation.** Where did the corpus come from, who digitised it, and who labelled the directories? Is there a citable publication, project page or dataset release from the Latin department? What licence and ethics language do we need? Plank 1 is weak without this and it gates the Dataset Construction rewrite.
