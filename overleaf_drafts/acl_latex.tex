...
The main comparison also includes SIF weighting \citep{arora2017sif}, which divides each token weight by its estimated corpus frequency and so down-weights high-frequency tokens before the passage vector is formed. SIF and ABTT act on the same problem at different stages. SIF reweights tokens before pooling; ABTT removes dominant directions after pooling. Token probabilities for SIF come from training files only. We report four settings throughout: baseline mean pooling, SIF alone, ABTT alone, and SIF followed by ABTT. As a third alternative, PCA whitening \citep{su2021whitening} rescales principal directions rather than only removing the top components. Whitening does not beat baseline assignment accuracy for any model at any layer in our grid, so we do not report it in the main tables.
...
The results move from diagnosis to use. Task A shows what each model can do at its best layer under each correction. The layerwise view then separates universal anisotropy from the T5-specific mid-depth collapse. Task B turns the corrected scores into a routing workflow and supplies the train-only layer rule for attribution, and we compare integrated gradients and retrieval-adapted MaRC at those locked layers.
...
The cosine gap column shows why. Under baseline pooling, even the models with strong AUROC score equivalent and non-equivalent passages within about 0.10 of each other, so the ranking is right but the score carries almost no margin. ABTT restores that margin for all six, which is what makes a single learned threshold usable in Task B.

\paragraph{SIF as a frequency-based complement.}
SIF and ABTT act on the same dominant direction from different sides, and Tables~\ref{tab:taskA_headline} and~\ref{tab:taskB_headline} show how far the frequency-based side gets. On Task B assignment accuracy, SIF alone adds 9.4 points for LaTa, 8.2 for PhilTa, and 7.3 for mT5-base, the three models whose retrieval collapses. It adds 0.1 point for LaBSE and subtracts 1.9 for KaLM-mini and 6.3 for Qwen3-0.6B. This fits the mechanism: a common component built mostly from frequent tokens can be attenuated by reweighting those tokens before pooling, but one that survives that reweighting needs the geometric step after pooling. SIF alone never matches ABTT on either task for any model, and adding SIF before ABTT moves best-layer assignment accuracy by at most 1.8 points in either direction. We read SIF as a partial account of the anisotropy rather than a competing repair, and keep the full SIF sweeps in Appendix~\ref{app:sif_variants}.
...
Table~\ref{tab:taskB_headline} reports best-layer autonomous routing. Baseline assignment accuracy spans 40.3 points, from 47.2 for mT5-base to 87.5 for KaLM-mini, so the choice of off-the-shelf encoder dominates the result. ABTT compresses that spread to 3.3 points, from 88.5 to 91.7, and lifts every model above every baseline. The largest single gain is 43.1 points for mT5-base and the smallest is 4.2 points for KaLM-mini. Directory accuracy at rank 1 moves from a 39.3-point spread to a 3.3-point spread.
...
Table~\ref{tab:attribution_metrics_main} reports the main attribution comparison, and Figure~\ref{fig:attribution_rho_loo_main} in Appendix~\ref{app:attribution_sweeps} plots the same rank-faithfulness column. The experiment uses 200 positive query-candidate pairs per model for LaTa, PhilTa, and mT5-base. Each model has baseline and ABTT variants for both integrated gradients and retrieval-adapted MaRC. The retrieval score being explained is repaired at these layers: at layer 7, LaTa moves from AUROC 0.499 and directory accuracy 0.302 to 0.961 and 0.868; at layer 1, PhilTa moves from 0.939 and 0.693 to 0.976 and 0.883, and mT5-base from 0.823 and 0.451 to 0.979 and 0.885.
...
The attribution results need a narrow reading. Baseline attribution explains raw cosine and ABTT attribution explains corrected cosine, so cross-variant comparisons are descriptive. The stable result is improved leave-one-out rank faithfulness in all six model-method cells; the ERASER-style metrics show threshold-dependent tradeoffs.
