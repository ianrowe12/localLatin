...
\caption{Cosine gap against normalized layer depth for all six models. The cosine gap is the mean cosine between passages that share a directory minus the mean cosine between passages that do not, the decision margin a similarity threshold has to work with, so larger is better. Gray is baseline mean pooling, blue is ABTT-only with $D$ tuned on train, orange is SIF-only. The baseline gap stays under 0.29 at every layer of every model, and under 0.11 for Qwen3-0.6B. ABTT lifts all six to a gap near 0.5 at every depth, including the middle layers where the T5 encoders in the top row lose their retrieval signal. SIF-only raises the gap for LaTa and PhilTa and leaves the other four close to baseline.}
\label{fig:gap}
\end{figure*}
...
\paragraph{PCA whitening (excluded).} PCA whitening \citep{su2021whitening} rescales principal directions rather than only removing the top components. On this corpus it flattens the same-directory margin along with the common component and degenerates the learned routing threshold, so we exclude it from the main tables and report the sweep in Appendix~\ref{app:sif_variants}.
...
The results move from diagnosis to use: Task A for every model and correction, the layerwise view, Task B routing under the train-only layer rule, then attribution at the locked layers.

\subsection{Task A: Pairwise Duplicate Detection}

\input{tables/taskA_headline}

Table~\ref{tab:taskA_headline} gives the best-layer Task A result for every model and setting. Baseline quality is uneven, with AUROC from 0.838 for mT5-base to 0.972 for KaLM-mini, and ABTT removes most of that unevenness: every model lands between 0.971 and 0.987, and the cosine gap rises from a baseline range of 0.024 to 0.237 to a range of 0.525 to 0.611. The gain is largest where the baseline is weakest: mT5-base gains 0.137 AUROC, LaTa 0.034, and KaLM-mini 0.009.
...
Character 3--5-gram TF-IDF matches the best ABTT cell on Task A and leads it on Task B routing, and fine-tuning LaTa reaches the routing accuracy of the zero-shot repair rather than exceeding it. What we report is where the retrieval signal lives inside these encoders and what recovers it, not a better retriever than string matching.

\paragraph{SIF as a frequency-based complement.}
SIF and ABTT act on the same dominant direction from different sides, and Tables~\ref{tab:taskA_headline} and~\ref{tab:taskB_headline} show how far the frequency side gets. On Task B assignment accuracy, SIF-only adds 9.4 points for LaTa, 8.2 for PhilTa, and 7.3 for mT5-base, the three models whose retrieval collapses; it adds 0.1 point for LaBSE and subtracts 1.9 for KaLM-mini and 6.3 for Qwen3-0.6B. This fits the mechanism. Reweighting attenuates a common component built from frequent tokens, but whatever survives it needs the geometric step after pooling. SIF-only never matches ABTT-only on either task for any model, and adding SIF before ABTT moves best-layer assignment accuracy by at most 1.8 points either way. We read SIF as a partial account of the anisotropy rather than a competing repair, and keep the full sweeps in Appendix~\ref{app:sif_variants}.
...
The score distributions agree. At PhilTa's collapsed retrieval layer, the mid-depth layer picked by training-set AUROC, equivalent and non-equivalent cosines overlap heavily before ABTT and separate after it (Figure~\ref{fig:density}, Appendix~\ref{app:score_distributions}).

Two-dimensional projections give a visual check. Under ABTT-only with $D=10$, same-directory points form tighter groups than under baseline mean pooling, with winnable-witness silhouette scores rising by 0.07 to 0.72 on the three T5 encoders. Appendix~\ref{app:cluster_geometry} has t-SNE \citep{vandermaaten2008tsne} and UMAP \citep{mcinnes2018umap} projections for all six models.

\subsection{Task B: Directory Routing}

Task B connects the layerwise analysis to the retrieval workflow through the two variants of Section~\ref{sec:tasks}: autonomous routing and top-$k$ ranking. Per-layer routing and ranking tables are in Appendices~\ref{app:per_layer_headline} and~\ref{app:per_layer_extra}.
...
Table~\ref{tab:taskb} in Appendix~\ref{app:sif_variants} repeats the top-1 comparison under five query and reference reseedings, so the compression is not an artifact of one draw. The baseline spread of 36.6 points, from 50.7 for mT5-base to 87.3 for KaLM-mini, falls to 1.0 point under the best per-model SIF plus ABTT configuration, from 89.52 for KaLM-mini to 90.55 for LaBSE, with standard deviations of at most 1.1 points. The same table gives the scholar-facing shortlist view: every model exceeds 95\% by Top-2 and gains little after Top-3, so two or three options cover nearly all successful cases.
...
Table~\ref{tab:attribution_metrics_main} reports the main attribution comparison: 200 positive query-candidate pairs per model for LaTa, PhilTa, and mT5-base, drawn from the same benchmark test split as every retrieval result above, with baseline and ABTT variants of both integrated gradients and retrieval-adapted MaRC. ABTT repairs the scores being explained at these layers: LaTa layer 7 moves from AUROC 0.498 to 0.962 and directory accuracy 0.302 to 0.868, PhilTa layer 1 from 0.939 to 0.977 and 0.693 to 0.883, and mT5-base layer 1 from 0.822 to 0.979 and 0.451 to 0.885.

Post-processing with ABTT improves rank faithfulness in five of six model-view cells ($\rho_{\mathrm{LOO}}$ 5/6, Kendall $\tau_b$ agreeing in the same five), with gains of 9.4 to 19.3 standard errors; the sixth cell is a tie at 1.0 standard error, not a loss. Chance-corrected deletion faithfulness improves in four of six. The sufficiency-side metrics are in the appendix rather than the main table because they fail our shuffled-attribution control in a baseline cell. We therefore claim that ABTT yields better-ranked rationales, not that it improves faithfulness on every axis.

Masking is applied at the representation level over cached layer-$L$ hidden states, so every faithfulness statement here is about the pooled read-out at that layer rather than about the encoder's input-to-output map. Appendix~\ref{app:attribution_sweeps} carries $\tau_b$, chance-corrected insertion faithfulness, the ERASER-style rationale metrics and their threshold sweeps (Table~\ref{tab:attribution_metrics_secondary}), and plots the rank-faithfulness column (Figure~\ref{fig:attribution_rho_loo_main}). Appendix~\ref{app:qual_attribution} shows example-level views for one PhilTa retrieval case, including the integrated-gradient pair matrix before and after ABTT.

\section{Discussion}

Six models across three architectures narrow what we can claim. Anisotropy is universal on this corpus and so is the repair, but the dramatic mid-depth collapse is not: only the T5 encoders in our set turn their anisotropy into a retrieval collapse. Published layerwise profiles place the mid-depth anisotropy peak in decoder-only models and do not separate encoder-decoder encoders \citep{razzhigaev2024shape, godey2024anisotropy}, so we read this as a first report for T5 encoders rather than a property of the architecture. A study of one architecture could easily report that collapse as the general finding; across three, the general finding is the geometry and the correction.

The practical reading depends on the architecture. With a T5 encoder the middle layers are unusable without correction, so layer choice matters there. With a non-T5 model the final layers already work, and the correction still adds 3.5 to 9.1 points of directory accuracy at rank 1, for one SVD per layer fit on 847 training files.

Geometry diagnoses the failure mode but does not select layers. Use it to find where representations collapse and recover, then pick retrieval layers with a train-only task signal. The two answers differ in our attribution experiment: the selected layers are not the most anisotropic ones.

The attribution results need a narrow reading. ABTT changes the cosine being explained, so the leave-one-out rank gain is the only broadly stable effect, and even it is a tie rather than a gain in one of six cells.
...
\caption{Cosine similarity distributions for PhilTa at the last layer and at its collapsed retrieval layer, before and after ABTT. Equivalent pairs share a directory; non-equivalent pairs do not. The collapsed retrieval layer is layer 8, the mid-depth layer with the highest training-set AUROC, so the panel understates the collapse rather than selecting its worst case; the same train-only rule that fixes every other choice in the paper picks it, and its test AUROC is 0.542, close to chance. Two other PhilTa layers carry other meanings and are not shown here: the most anisotropic layer under the top-PC rule of Appendix~\ref{app:layer_diagnostics} is layer 6, and the lowest baseline test AUROC, 0.538, falls at layer 10. Before ABTT the two distributions sit almost on top of each other, and ABTT separates them.}
\label{fig:density}
\end{figure}
