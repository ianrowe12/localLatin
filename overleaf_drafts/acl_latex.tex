...
(1) we introduce an expert-labelled benchmark for semantic matching in Medieval Latin canon law, built on the source keys that the Carolingian Canon Law project assigned and verified, together with a routing task that states the scholar's decision directly: file an unidentified witness under a known fragment, or flag it as new;
...
The labels are expert-assigned, but by one project. Its director or supervised graduate students assigned the keys, and verification is by collation in the project's parallel display, not by independent double annotation, so no inter-annotator agreement figure exists for this benchmark.

\section*{Ethical Considerations}

The benchmark holds early medieval legal texts, transcribed from manuscripts and early printings, and contains no personal data. The Carolingian Canon Law project publishes its material under a Creative Commons Attribution-NonCommercial-ShareAlike 4.0 licence \citep{ccl2026about}. The project's director has confirmed in writing that the licence covers the transcriptions, annotations and markup in the exports we use and that, in the view of the project's Advisory Board, attribution to the project suffices; the project's interface credits the transcriber and proofreader of each unit. The project's corpus keeps growing and being corrected, so benchmark version 1 is a dated snapshot, fixed by a sha256 manifest of its 1,705 labelled files, and every number in this paper comes from it. We will release it under the same licence, in coordination with the project's periodic versioned exports, and later versions will follow those exports. The intended use is research on retrieval and on assisted source identification: the system ranks candidates for an expert to judge and does not identify sources on its own.
...
%   * Appendix A: drop the "written communication" footnote (its source is
%     then an author) and name the director where the text says "the
%     project's director".
%
